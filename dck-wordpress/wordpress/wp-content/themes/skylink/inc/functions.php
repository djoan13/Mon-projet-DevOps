...
function skylink_page_builder_components($components)
{
...
function skylink_default_components($components)
{
...
function skylink_register_components($components = array())
{
    if (apply_filters('colibri_page_builder/installed', false)) {
        $components = skylink_page_builder_components($components);
    } else {
        $components = skylink_default_components($components);
    }

    return $components;
}

Hooks::prefixed_add_action('components', 'skylink_register_components');
Theme::load(array(
    'themeRelativePath' 		=> '',
 ...
 */
function skylink_assets()
{
    return skylink_theme()->getAssetsManager();
}
...
add_action('after_setup_theme', function() {
    skylink_theme()->register_menus(array(
        'header-menu' => esc_html__('Header Menu', 'skylink'),
        'footer-menu' => esc_html__('Footer Menu', 'skylink'),
    ));
...
add_action('widgets_init', 'skylink_register_sidebars');
function skylink_register_sidebars()
{
    register_sidebar(array(
        'name' 			=> esc_html__('Blog sidebar widget area', 'skylink'),
    ...
    if (function_exists('\is_shop')) {
        register_sidebar(array(
            'name' 			=> esc_html__('Woo Commerce left sidebar widget area', 'skylink'),
            'id' 			=> 'skylink-ecommerce-left',
            'before_widget' => '<div id="%1$s" class="widget %2$s">',
            'before_title' 	=> '<h5 class="widgettitle">',
            'after_title' 	=> '</h5>',
            'after_widget' 	=> '</div>',
    ...
    if (!apply_filters('colibri_page_builder/installed', false)) {
        skylink_assets()
            ...
            ->registerStylesheet("skylink-theme", "/theme/theme.css")
    ...
    skylink_assets()->registerTemplateStyle('skylink-theme-extras', '/theme/extras.css', 'skylink-theme');
...
//blog options
    ...
    function skylink_show_post_meta_setting_filter($value)
    {
    ...
    add_filter('colibri_show_post_meta', 'skylink_show_post_meta_setting_filter');


    function skylink_posts_per_row_setting_filter($value)
    {
    ...
    add_filter('colibri_posts_per_row', 'skylink_posts_per_row_setting_filter');

    function skylink_archive_post_highlight_setting_filter($value)
    {
    ...
    add_filter('colibri_archive_post_highlight', 'skylink_archive_post_highlight_setting_filter');


    function skylink_blog_sidebar_enabled_setting_filter($value)
    {
    ...
    Hooks::prefixed_add_filter('blog_sidebar_enabled', 'skylink_blog_sidebar_enabled_setting_filter');
//add_filter( 'blog_sidebar_enabled', 'skylink_blog_sidebar_enabled_setting_filter' );

    function skylink_override_with_thumbnail_image($value)
    {
        global $post;
    ...
    add_filter('colibri_override_with_thumbnail_image', 'skylink_override_with_thumbnail_image');

    function skylink_print_archive_entry_class($class = "")
    {
        ...
        $postsPerRow = max(1, apply_filters('skylink_posts_per_row', $default));


        $classes[] = "h-col-sm-12 h-col-md-" . (12 / intval($postsPerRow));

        $classes = apply_filters('skylink_archive_entry_class', $classes);
    ...
    function skylink_print_masonry_col_class($echo = false)
    {

        global $wp_query;
        $index = $wp_query->current_post;
        $hasBigClass = (is_sticky() || ($index === 0 && apply_filters('skylink_archive_post_highlight', false)));
    ...
    Hooks::prefixed_add_filter('info_page_tabs', 'skylink_get_started_info_page_tab');

    function skylink_get_started_info_page_tab($tabs)
    {
    ...
    function skylink_theme_plugins($plugins)
    {
        $theme_plugins = array();
    ...
    Hooks::prefixed_add_filter('theme_plugins', 'skylink_theme_plugins');


    add_filter('http_request_host_is_external', 'skylink_allow_internal_host', 10, 3);
    function skylink_allow_internal_host($allow, $host, $url)
    {
    ...
    add_action('wp_ajax_skylink_front_set_predesign', function () {
        check_ajax_referer( 'skylink_front_set_predesign_nonce', 'nonce' );
    ...
    function skylink_override_main_row_class($classes)
    {
        return Defaults::get('templates.blog.row.layout-classes', $classes);
    }

    Hooks::prefixed_add_filter('main_row_class', 'skylink_override_main_row_class', 10, 1);
...
function skylink_has_fresh_site() {
	$mods = get_theme_mods();
