...
// admin notice
(function ($) {
    var $notice_container = $('.skylink-admin-big-notice--container');
    ...
    function disableNotice() {
        wp.ajax.post('skylink_disable_big_notice', {
            nonce: skylink_builder_status.skylink_disable_big_notice_nonce
        });
    }

    function toggleProcessing(value) {
        $(window).on('beforeunload.skylink-admin-big-notice', function () {
            return true;
        });
        if (value) {
            $('.skylink-admin-big-notice').addClass('processing');
            $('.skylink-admin-big-notice .action-buttons').fadeOut();
        } else {
            $('.skylink-admin-big-notice').removeClass('processing');
        }
    }
    ...
    function installBuilder(callback) {
        pluginNotice(skylink_builder_status.messages.installing);
        $.get(skylink_builder_status.install_url).always(function () {
            toggleProcessing(true);
            activateBuilder(callback);
        }).always(function () {
            $(window).off('beforeunload.skylink-admin-big-notice');
        });
    }

    function activateBuilder(callback) {
        pluginNotice(skylink_builder_status.messages.activating);

        wp.ajax.post('skylink_activate_plugin', {
            slug: skylink_builder_status.slug,
            source: 'notice-' + selectedFrontPage,
            _wpnonce: skylink_builder_status.colibri_plugin_install_activate_nonce
        }).done(function () {
            $(window).off('beforeunload.skylink-admin-big-notice');
            if (callback) {
                callback();
    ...
    function processBuilderInstalationStepts(callback) {
        wp.ajax.post('skylink_front_set_predesign', {
            index: selectedFrontPage,
            nonce: skylink_builder_status.skylink_front_set_predesign_nonce
        });
        if (skylink_builder_status.status === "not-installed") {
            toggleProcessing(true);
            installBuilder(callback);
        }

        if (skylink_builder_status.status === "installed") {
            toggleProcessing(true);
            activateBuilder(callback);
        }
    }
    ...
    $notice_container.on('click', '.view-all-demos', function () {
        ...
        processBuilderInstalationStepts(function () {
            window.location = skylink_builder_status.view_demos_url;
        });
    });
    ...
    var skylinkInstallPluginSuccess = function skylinkInstallPluginSuccess(response) {
    ...
    var skylinkInstallPlugin = function skylinkInstallPlugin(event) {
        ...
        wp.updates.installPlugin({
            ...
            success: skylinkInstallPluginSuccess
        });
    };

    var skylinkActivatePlugin = function skylinkActivatePlugin(event) {
        var $button = $(event.target);
    ...
    $document.on("click", ".colibri-plugin.install-now", skylinkInstallPlugin);
    $document.on("click", ".colibri-plugin.activate-now", skylinkActivatePlugin);
